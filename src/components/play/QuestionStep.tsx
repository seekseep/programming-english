  ...
  return (
    <main className="px-4 py-4 min-h-dvh flex flex-col">
      <div className="flex flex-col flex-1">
        {/* ヘッダー */}
        <div className="flex items-center gap-3 mb-6 max-w-xl w-full mx-auto">
          <button
            type="button"
            onClick={requestExit}
            className="cursor-pointer p-1 text-muted"
          >
            <X size={24} />
          </button>
        ...
        <div
          className={cn(
            'flex-1 flex items-center justify-center text-center transition-all duration-300 ease-out',
        ...
        <div className="flex flex-col gap-3 mt-auto max-w-xl w-full mx-auto">
          {choices.map((choice, i) => {
            ...
            const isWrongSelected = !!selected && choice.japanese === selected && !isCorrectChoice
            return (
              <button
                key={choice.english}
                type="button"
                ...
                className={cn(
                  ...
                  selected && isCorrectChoice ? 'bg-(--answer-correct-bg)'
                    : isWrongSelected ? 'bg-(--answer-incorrect-bg)'
                    : 'bg-(--card-bg)',
                )}
                style={{ transitionDelay: phase === 'visible' ? `${i * 50}ms` : undefined }}
              >
                {choice.japanese}
              </button>
            )
